...
import PsychologicalQuestionEditModal from './psychological-question-edit-modal';
import { getImageUrl } from '@/utils/image-utils';

interface PsychologicalQuestionsListProps {
  questions: PsychologicalQuestionWithOptions[];
}

const ITEMS_PER_PAGE = 10;

export default function PsychologicalQuestionsList({
  questions,
}: PsychologicalQuestionsListProps) {
  const [selectedQuestion, setSelectedQuestion] =
    useState<PsychologicalQuestionWithOptions | null>(null);
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [isCreateModalOpen, setIsCreateModalOpen] = useState(false);
  const [filterType, setFilterType] = useState<'all' | 'real' | 'educational'>(
    'all'
  );
  ...
  const filteredQuestions = useMemo(() => {
    ...
    if (filterType === 'real')
      return questions.filter((q) => q.isRealPsychological);
    return questions.filter((q) => !q.isRealPsychological);
  ...
  const handleFilterChange = (value: 'all' | 'real' | 'educational') => {
    setFilterType(value);
    setCurrentPage(1);
  };

  const handleEdit = (question: PsychologicalQuestionWithOptions) => {
  ...
  return (
    <>
      <div className="mb-8">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-4 items-center">
            <label className="text-sm font-medium text-[#1A2229]">Տիպ:</label>
            <select
              value={filterType}
              onChange={(e) =>
                handleFilterChange(
                  e.target.value as 'all' | 'real' | 'educational'
                )
              }
              className="border border-gray-300 rounded-[10px] px-4 py-2 focus:outline-none focus:border-[#FA8604] max-w-[200px]"
            >
              <option value="all">Բոլորը</option>
              <option value="real">Իրական հոգեբանական</option>
              <option value="educational">Ուսուցողական</option>
            </select>
          </div>
        </div>
      </div>
      ...
      {(isEditModalOpen || isCreateModalOpen) && (
        <PsychologicalQuestionEditModal
